"""Single-script: spawn fresh E2B sandbox, install everything, render short video."""
import os, sys, time
from e2b import Sandbox

os.environ["E2B_API_KEY"] = "e2b_e211aca6616cd7e18155af3973539bf7b9bd7772"

# Short composition - 5 seconds, 5 scenes x 1s each
html = r"""<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
print("=== Create fresh sandbox ===", flush=True)
sbx = Sandbox.create(timeout=1800)  # 30 min total sandbox TTL
...
try:
    ...
    print("\n=== Install Node 22 + ffmpeg + Chrome deps + swap ===", flush=True)
    r = sbx.commands.run(
        "sudo fallocate -l 2G /swapfile && sudo chmod 600 /swapfile && sudo mkswap /swapfile && sudo swapon /swapfile && "
        "curl -fsSL https://deb.nodesource.com/setup_22.x | sudo -E bash - > /tmp/nodesetup.log 2>&1 && "
    ...
    # 2. Verify Node + install HyperFrames + Chrome
    print("\n=== Install HyperFrames + Chrome ===", flush=True)
    r = sbx.commands.run(PATHFIX + "node --version && npm install -g hyperframes 2>&1 | tail -3", timeout=180)
    print(r.stdout, flush=True)

    r = sbx.commands.run(PATHFIX + "hyperframes browser ensure 2>&1 | tail -5", timeout=240)
    print(r.stdout[-500:], flush=True)

    # 3. Write HTML composition
    print("\n=== Write composition ===", flush=True)
    ...
    print("\n=== Start render (background) ===", flush=True)
    r = sbx.commands.run(
    ...
    # 5. Poll for completion (max 8 min)
    print("\n=== Poll for render ===", flush=True)
    start = time.time()
    done = False
    while time.time() - start < 480:  # 8 min
        time.sleep(20)
        try:
            r = sbx.commands.run(
            ...
            print(f"[{elapsed}s] {r.stdout.strip()}", flush=True)
    ...
    if not done:
        print("Render did not complete in time. Last log:", flush=True)
        try:
    ...
    with open("/tmp/e2b_sandbox_id.txt", "w") as f:
        f.write(sbx_id)
    print(f"\nFinal sandbox ID: {sbx_id}", flush=True)
