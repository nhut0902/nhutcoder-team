...
"""Write Remotion composition files + npm install + start render. All in one script."""
import os, time, json
...
print(f"Total: {total_dur:.2f}s = {total_frames} frames at {FPS}fps", flush=True)

# Write package.json
print("\n=== Write package.json ===", flush=True)
sbx.files.write("/home/user/kimi_project/package.json", """{
  "name": "kimi-k27-video",
  "version": "1.0.0",
  "dependencies": {
    "react": "^19.0.0",
...
# Write tsconfig.json
sbx.files.write("/home/user/kimi_project/tsconfig.json", """{
  "compilerOptions": {
    "target": "ES2020",
    "module": "ESNext",
...
# Write index.tsx
print("\n=== Write index.tsx ===", flush=True)
index_tsx = f'''import {{Composition, staticFile, AbsoluteFill, Audio, Sequence, Img, useCurrentFrame, interpolate, Easing, registerRoot}} from 'remotion';
import React from 'react';

const FPS = {FPS};
const W = 1080;
const H = 1920;
const TOTAL_DURATION = {total_frames};

const SCENES = [
  {{id: 's1', start: 0, duration: {int(durations['s1'] * FPS)}, comp: Scene1}},
  {{id: 's2', start: {int(scene_starts['s2'] * FPS)}, duration: {int(durations['s2'] * FPS)}, comp: Scene2}},
  {{id: 's3', start: {int(scene_starts['s3'] * FPS)}, duration: {int(durations['s3'] * FPS)}, comp: Scene3}},
  {{id: 's4', start: {int(scene_starts['s4'] * FPS)}, duration: {int(durations['s4'] * FPS)}, comp: Scene4}},
  {{id: 's5', start: {int(scene_starts['s5'] * FPS)}, duration: {int(durations['s5'] * FPS)}, comp: Scene5}},
  {{id: 's6', start: {int(scene_starts['s6'] * FPS)}, duration: {int(durations['s6'] * FPS)}, comp: Scene6}},
  {{id: 's7', start: {int(scene_starts['s7'] * FPS)}, duration: {int(durations['s7'] * FPS)}, comp: Scene7}},
  {{id: 's8', start: {int(scene_starts['s8'] * FPS)}, duration: {int(durations['s8'] * FPS)}, comp: Scene8}},
];
...
const Subtitle: React.FC<{{lines: string[]}}> = ({{lines}}) => (
  <div style={{{{position: 'absolute', bottom: 180, left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center'}}}}>
    {{lines.map((line, i) => (
      <div key={{i}} style={{{{color: '#f5f5fa', fontSize: 38, fontWeight: 500, textAlign: 'center', marginBottom: 10}}}}>{{line}}</div>
    )))}}
  </div>
);
...
const Scene5: React.FC = () => {{
  ...
  return (
    <AbsoluteFill style={{{{...entr, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 60}}}}>
      ...
      <div style={{{{fontSize: 50, fontWeight: 800, color: '#9b72dc', marginTop: 130, marginBottom: 30}}}}>moonshotai/Kimi-K2.7-Code</div>
...
'''
sbx.files.write("/home/user/kimi_project/src/index.tsx", index_tsx)
print("  src/index.tsx written", flush=True)

# npm install
print("\n=== npm install ===", flush=True)
r = sbx.commands.run(PATHFIX + "cd /home/user/kimi_project && npm install --no-audit --no-fund 2>&1 | tail -3", timeout=300)
...
r = sbx.commands.run(
    ...
    "(export PATH=/usr/bin:$PATH && nohup remotion render src/index.tsx KimiVideo out/kimi.mp4 --codec h264 --crf 23 --concurrency 1 > /tmp/render.log 2>&1 < /dev/null &) "
    "&& sleep 2 && echo RENDER_STARTED",
    timeout=30,
)
print(r.stdout, flush=True)
...
# Wait 30s then check
time.sleep(30)
r = sbx.commands.run("ls /home/user/kimi_project/out/*.mp4 2>/dev/null && echo MP4_FOUND || echo NO_MP4; ps aux | grep remotion | grep -v grep | wc -l; tail -5 /tmp/render.log 2>&1 | head -c 500", timeout=15)
print("\n30s check:", r.stdout, flush=True)

# Poll
print("\n=== Poll (max 10 min) ===", flush=True)
